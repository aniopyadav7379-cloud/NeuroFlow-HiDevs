...
from backend.security.sanitization import ValidationError, validate_file_signature, validate_url_for_ssrf
from backend.resilience.rate_limit_dependency import rate_limit
from backend.security.auth import require_scope
...
@router.post(
    "/ingest", response_model=IngestResponse, status_code=202,
    dependencies=[Depends(INGEST_RATE_LIMIT), Depends(require_scope("ingest"))],
)
async def ingest(request: Request, file: UploadFile | None = File(default=None)) -> IngestResponse:
    ...
    if file is not None:
        ...
        try:
            validate_file_signature(content_bytes, source_type)
...
@router.post(
    "/ingest/url", response_model=IngestResponse, status_code=202,
    dependencies=[Depends(INGEST_RATE_LIMIT), Depends(require_scope("ingest"))],
)
