...
from backend.security.prompt_injection import classify_query, scan_patterns
from backend.security.sanitization import ValidationError, clean_text, validate_query_length
...
async def query(request: Request, body: QueryRequest):
    ...
    try:
        validate_query_length(body.query)
    except ValidationError as e:
        raise HTTPException(status_code=400, detail=str(e))
    body.query = clean_text(body.query)

    # Layer 1 (pattern match) — flag only, never blocks; logged for
    # monitoring regardless of layer 2's outcome.
    pattern_hit = scan_patterns(body.query)
    if pattern_hit:
        logger.warning("query flagged by injection pattern: pipeline_id=%s pattern=%r", body.pipeline_id, pattern_hit["pattern"])

    # Layer 2 (LLM classification) — blocks.
    if await classify_query(body.query, llm_client):
