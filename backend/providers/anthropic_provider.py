...
def _convert_content_block(block: dict) -> dict:
    """ChatMessage content lists are constructed in OpenAI's multimodal
    shape (see pipelines/ingestion/extractors/vision.py) since that's the
    common case NeuroFlow builds. Anthropic's text blocks are shape-
    compatible ({"type": "text", "text": ...}) but images aren't — Anthropic
    wants {"type": "image", "source": {"type": "base64", "media_type": ...,
    "data": ...}} instead of an OpenAI data-URL image_url block. This
    converts just that one shape; anything else passes through unchanged.
    """
    if block.get("type") != "image_url":
        return block
    data_url: str = block["image_url"]["url"]
    if not data_url.startswith("data:"):
        raise NonRetryableProviderError(
            "AnthropicProvider only supports base64 data-URL images, not remote image URLs"
        )
...
def _convert_content(content: str | list) -> str | list:
    if isinstance(content, str):
        return content
    return [_convert_content_block(block) for block in content]
...
def _split_system_and_messages(messages: list[ChatMessage]) -> tuple[str | None, list[dict]]:
    ...
    for m in messages:
        if m.role == "system":
            ...
            converted.append({"role": m.role, "content": _convert_content(m.content)})
