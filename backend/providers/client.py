...
class NeuroFlowClient:
    ...
    async def chat(self, messages: list[ChatMessage], routing_criteria: RoutingCriteria, **kwargs) -> GenerationResult:
        model_config = await self._router.select_model(routing_criteria)
        ...
        with tracer.start_as_current_span("neuroflow.provider.chat") as span:
            ...
            result = await provider.complete(messages, model=model_config.model, **kwargs)
    ...
    async def stream(self, messages: list[ChatMessage], routing_criteria: RoutingCriteria, **kwargs) -> AsyncGenerator[str, None]:
        ...
        with tracer.start_as_current_span("neuroflow.provider.stream") as span:
            ...
            started = time.perf_counter()
            token_count = 0
            async for token in provider.stream(messages, model=model_config.model, **kwargs):
                token_count += 1
                yield token
    ...
    async def embed(self, texts: list[str], routing_criteria: RoutingCriteria | None = None) -> list[list[float]]:
        ...
        with tracer.start_as_current_span("neuroflow.provider.embed") as span:
            ...
            vectors = await provider.embed(texts, model=model_config.model)
