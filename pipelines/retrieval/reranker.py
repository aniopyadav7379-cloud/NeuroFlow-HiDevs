...
async def rerank(
    ...
    with _tracer.start_as_current_span("retrieval.rerank") as span:
        ...
        result = await _rerank_inner(query, candidates, scorer, top_k)
        span.set_attribute("output_chunk_count", len(result))
        return result


async def _rerank_inner(query, candidates, scorer, top_k):
    if not candidates:
        return []
