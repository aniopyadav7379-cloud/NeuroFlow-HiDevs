...
class Retriever:
    ...
    async def _dense_retrieval(self, query_texts: list[str], k: int) -> list[RetrievalResult]:
        ...
        with _tracer.start_as_current_span("retrieval.dense") as span:
            span.set_attribute("query_variant_count", len(query_texts))
            return await self._dense_retrieval_inner(query_texts, k, span)

    async def _dense_retrieval_inner(self, query_texts: list[str], k: int, span) -> list[RetrievalResult]:
        if not query_texts:
            return []
        ...
        retrieval_latency.labels(strategy="dense").observe(_time.perf_counter() - _started)
        merged = _union_by_max_score(list(per_query_results))
        span.set_attribute("chunk_count", len(merged))
        return merged

    async def _sparse_retrieval(self, query: str, k: int) -> list[RetrievalResult]:
        with _tracer.start_as_current_span("retrieval.sparse") as span:
            result = await self._sparse_retrieval_inner(query, k, span)
            span.set_attribute("chunk_count", len(result))
            return result

    async def _sparse_retrieval_inner(self, query: str, k: int, span) -> list[RetrievalResult]:
        _started = _time.perf_counter()
        async with self.pool.acquire() as conn:
            rows = await conn.fetch(
                """
        ...
        retrieval_latency.labels(strategy="sparse").observe(_time.perf_counter() - _started)
    ...
    async def _metadata_retrieval(self, query: str, filters: dict, k: int) -> list[RetrievalResult]:
        ...
        with _tracer.start_as_current_span("retrieval.metadata") as span:
            span.set_attribute("filters_present", bool(filters))
            result = await self._metadata_retrieval_inner(query, filters, k)
            span.set_attribute("chunk_count", len(result))
            return result

    async def _metadata_retrieval_inner(self, query: str, filters: dict, k: int) -> list[RetrievalResult]:
        if not filters:
            return []
        _started = _time.perf_counter()
        ...
        retrieval_latency.labels(strategy="metadata").observe(_time.perf_counter() - _started)
