...
def assemble_context(
    ...
    with _tracer.start_as_current_span("retrieval.assemble") as span:
        span.set_attribute("candidate_chunk_count", len(chunks))
        span.set_attribute("token_budget", token_budget)
        result = _assemble_context_inner(chunks, token_budget)
        span.set_attribute("chunks_used_count", len(result.chunks_used))
        span.set_attribute("total_tokens", result.total_tokens)
        return result


def _assemble_context_inner(chunks, token_budget: int) -> AssembledContext:
    parts: list[str] = []
