...
async def process_query(
    query: str, client: NeuroFlowClient | None, *, enable_expansion: bool = True
) -> ProcessedQuery:
    ...
    `enable_expansion=False` (from a pipeline's retrieval.query_expansion
    config) skips the expansion LLM call entirely rather than calling it
    and discarding the result — saves the cost, not just the code path."""
    if enable_expansion:
        ...
        expanded = []
        query_type = await classify_query_type(query, client)
