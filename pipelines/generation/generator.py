...
async def create_pipeline_run(pool: asyncpg.Pool, pipeline_id: str, query: str) -> str:
    ...
    client right away for streaming requests."""
...
async def generate(
    *,
    ...
    after the LLM call, per the task's 6 steps."""
    use_cot = use_chain_of_thought and query_type in CHAIN_OF_THOUGHT_QUERY_TYPES
    messages = build_messages(query, query_type, assembled.context, use_chain_of_thought=use_cot)
    ...
    async for delta in llm_client.stream(messages, routing_criteria, model_config=model_config):
