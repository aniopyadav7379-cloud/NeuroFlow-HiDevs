...
from backend.monitoring.metrics import generation_latency, queries_total

logger = logging.getLogger("neuroflow.generation.generator")

from opentelemetry import trace
_tracer = trace.get_tracer("neuroflow.generation")
...
async def mark_run_failed(pool: asyncpg.Pool, run_id: str, error: str = "", pipeline_id: str | None = None) -> None:
    async with pool.acquire() as conn:
        await conn.execute(
...
async def generate(
    *,
    ...
    with _tracer.start_as_current_span("generation.prompt_build") as pb_span:
        pb_span.set_attribute("run_id", run_id)
        pb_span.set_attribute("pipeline_id", pipeline_id)
        pb_span.set_attribute("query_type", query_type)
    ...
    with _tracer.start_as_current_span("generation.llm_call") as llm_span:
        llm_span.set_attribute("run_id", run_id)
        llm_span.set_attribute("model", model_config.model)
    ...
    with _tracer.start_as_current_span("generation.citation_parse") as cp_span:
        cp_span.set_attribute("run_id", run_id)
        citations = parse_citations(full_response, assembled.sources)
        cp_span.set_attribute("citation_count", len(citations))
    ...
    with _tracer.start_as_current_span("generation.log_run") as log_span:
        log_span.set_attribute("run_id", run_id)
        log_span.set_attribute("input_tokens", input_tokens)
        log_span.set_attribute("output_tokens", output_tokens)
        async with pool.acquire() as conn:
            await conn.execute(
                """
                UPDATE pipeline_runs
    ...
    generation_latency.labels(model=model_config.model).observe(latency_ms / 1000.0)
    queries_total.labels(pipeline_id=pipeline_id, status="complete").inc()
