...
class StreamingGenerator:
    ...
    async def generate_stream(
        ...
        with tracer.start_as_current_span("generation.pipeline") as pipeline_span:
            ...
            # NOTE: everything from here through the final `yield ("", citations)` is
            # wrapped in try/except so that if the caller stops iterating this
            # generator early (e.g. a client disconnects mid-stream, or the request
            # is cancelled), the run is marked "failed" instead of being left stuck
            # at "running" forever. Without this, GeneratorExit (raised into this
            # generator's current `yield` when aclose() is called - which is exactly
            # what happens when a FastAPI SSE response's consumer goes away) would
            # propagate straight out, skipping the "status = 'complete'" UPDATE below
            # entirely, with no record that anything went wrong.
            try:
                stream_gen = await self.client.stream_chat(
                    messages, criteria, temperature=temperature
                )
                ...
                with tracer.start_as_current_span("generation.llm_call") as llm_span:
                    ...
                    lm_calls_total.labels(
                        provider="routed", model="routed", task_type=task_type
                    ).inc()
                ...
                metadata_json = (
                    json.dumps({"think_content": think_content}) if think_content else "{}"
                )
                ...
                # The consumer (e.g. the SSE endpoint) stopped iterating before we
                # finished - most commonly a client disconnect mid-stream. Mark the
                # run as failed rather than leaving it stuck at "running" forever,
                # then re-raise: an async generator must not swallow GeneratorExit.
                try:
                    async with self.db_pool.acquire() as conn:
                        await conn.execute(
                            """
                            ...
                            """,
                            json.dumps({"error": "stream aborted before completion"}),
                            int((time.time() - start_time) * 1000),
                            UUID(run_id),
                        )
                except Exception:
                    # Best-effort cleanup - never mask the original abort with a
                    # secondary DB error.
                    pass
                raise
