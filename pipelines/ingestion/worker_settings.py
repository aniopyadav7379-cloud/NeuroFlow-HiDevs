"""
arq WorkerSettings for the ingestion queue. Run via backend/worker.py
(the entrypoint infra/docker-compose.yml's `worker` service invokes as
`python -m worker`), or directly with `arq pipelines.ingestion.worker_settings.WorkerSettings`.
"""
import logging
...
logger = logging.getLogger("neuroflow.worker.ingestion")
...
async def startup(ctx: dict) -> None:
    ...
    try:
        ...
        logger.error("LLM client not initialized in ingestion worker: %s", e)
        ctx["llm_client"] = None
