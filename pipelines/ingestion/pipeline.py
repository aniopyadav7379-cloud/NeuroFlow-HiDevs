...
async def _embed_and_persist(pool, document_id: str, chunks: list[Chunk], llm_client: NeuroFlowClient) -> int:
    ...
    Every chunk is scanned for secrets and redacted BEFORE embedding —
    embedding a raw secret and only redacting it in storage afterward
    would still leak it into the vector itself. Injection-pattern
    scanning (flag-only, never blocks — see backend/security/
    prompt_injection.py's module docstring for why ingested documents
    are never rejected on this) happens on the same pass.
    """
    from backend.security.prompt_injection import scan_patterns
    from backend.security.secrets_detection import scan_and_redact

    embedding_calls = 0

    for chunk in chunks:
        redacted_text, secret_findings = scan_and_redact(chunk.text)
        if secret_findings:
            logger.warning(
                "document_id=%s: %d secret(s) redacted from chunk before embedding",
                document_id, len(secret_findings),
            )
            chunk.text = redacted_text
            chunk.metadata["secrets_redacted"] = [f["pattern_type"] for f in secret_findings]

        injection_hit = scan_patterns(chunk.text)
        if injection_hit:
            chunk.metadata.update(injection_hit)
    ...
    with tracer.start_as_current_span("ingestion.process") as span:
        ...
        try:
            with tracer.start_as_current_span(f"ingestion.extract.{source_type}") as extract_span:
                extract_span.set_attribute("document_id", document_id)
            ...
            with tracer.start_as_current_span("ingestion.chunk") as chunk_span:
                chunk_span.set_attribute("document_id", document_id)
            ...
            with tracer.start_as_current_span("ingestion.write_db") as db_span:
                ...
                db_span.set_attribute("chunk_count", len(chunks))
                async with pool.acquire() as conn:
                    await conn.execute(
