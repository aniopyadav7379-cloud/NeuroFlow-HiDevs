...
EVALUATIONS_CHANNEL = "evaluations:new"  # backend/api/evaluations.py's SSE feed subscribes here
...
class EvaluationJudge:
    ...
    async def _get_pipeline_id(self, run_id: str) -> str | None:
        async with self.pool.acquire() as conn:
            row = await conn.fetchval("SELECT pipeline_id FROM pipeline_runs WHERE id = $1", run_id)
    ...
    async def _write_evaluation(self, run_id: str, scores: EvaluationScores, judge_model: str, metadata: dict) -> str:
        async with self.pool.acquire() as conn:
            ...
            # Joined here (not passed in) so every caller of evaluate()
            # doesn't need to separately look up pipeline identity just to
            # satisfy the live feed — this is the one place that needs it.
            context_row = await conn.fetchrow(
                """
                SELECT r.pipeline_id, r.query, p.name AS pipeline_name
        ...
        await self._publish_feed_event(run_id, scores, row["evaluated_at"], context_row)
        return str(row["id"])

    async def _publish_feed_event(self, run_id: str, scores: EvaluationScores, evaluated_at, context_row) -> None:
        """Best-effort: GET /evaluations/stream (backend/api/evaluations.py)
        is a live dashboard feed, not a durability-critical path — a
        failed publish here should never fail the evaluation itself."""
        redis_client = getattr(self.llm_client, "redis", None)
        if redis_client is None or context_row is None:
            return
        try:
            payload = {
                "run_id": run_id,
                "pipeline_id": str(context_row["pipeline_id"]) if context_row["pipeline_id"] else None,
                "pipeline_name": context_row["pipeline_name"],
            ...
            await redis_client.publish(EVALUATIONS_CHANNEL, json.dumps(payload))
        except Exception:
            logger.warning("failed to publish evaluation feed event for run_id=%s (non-fatal)", run_id, exc_info=True)
    ...
    async def evaluate(
        ...
        pipeline_id = await self._get_pipeline_id(run_id)
        if pipeline_id:
            eval_faithfulness.labels(pipeline_id=pipeline_id).set(scores.faithfulness)
            eval_overall.labels(pipeline_id=pipeline_id).set(scores.overall_score)
